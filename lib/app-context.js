...
AppContext.createContext = function(contextSpec) {
  ...
  if (!contextSpec.configure || typeof(contextSpec.configure) !== 'function') throw new Error('Must pass an object with a configure method to AppContext.createContext.');
...
AppContext.loadFromPackage = function(runLevel, env, opts) {
  return q().then(function() {
    ...
    if (!package['app-context']) throw new Error('The package.json for your project does not have a "app-context" defined.');
  
    var context = require(path.join(path.dirname(packageFile), package['app-context']));
